...
is also compatible with the `minid` file manifest format and we will be using the output generated by this command to feed the `minid` CLI command `batch_register` in a moment.
